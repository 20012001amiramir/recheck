...
func TestSchemaStrictness(t *testing.T) {
	...
	cases := []struct {
		...
		{"registry reason outside the closed list", `"method": null`, `"method": null, "reason": "something_else"`, "$.claims[1].exists.registry.reason"},
	...
	accept := []struct {
		...
		{"the case registry's reason year_unreadable", [][]string{{`"method": null`, `"method": "search", "reason": "year_unreadable"`}}},
		{"the case registry's reason registry_budget", [][]string{{`"method": null`, `"method": "search", "reason": "registry_budget"`}}},
		{"the case registry's reason citation_belongs_to_another_case", [][]string{{`"method": null`, `"method": "lookup", "reason": "citation_belongs_to_another_case", "name_check": "mismatch", "cluster_id": 853092`}}},
		{"the case registry's reason recent_volume", [][]string{{`"method": null`, `"method": "name_search", "reason": "recent_volume"`}}},
